...
inline void MakePersonalPasswordDataFile(const std::filesystem::path& UniqueFileNamePath, const std::string& Token)
{
	if(!std::filesystem::is_directory(UniqueFileNamePath.parent_path()))
	{
		// 如果目录不存在，创建它
		if (!std::filesystem::create_directories(UniqueFileNamePath.parent_path()))
		{
	...
	if (std::filesystem::exists(UniqueFileNamePath))
	{
	...
	RegenerateMasterKey(UniqueFileNamePath, PersonalPasswordInfo, Token);

	PersonalPasswordInfo.Serialization(UniqueFileNamePath);

	LogInfoHelper("Personal password data file created: {0}", std::make_tuple(UniqueFileNamePath.string()));
}
...
inline void FirstLoginLogic(const std::vector<char>& BufferLoginPassword, const PasswordManagerUserKey& CurrentUserKey, PasswordManagerUserData& CurrentUserData)
{
	...
	MakePersonalPasswordDataFile(std::string("PersonalPasswordData/") + "Passwords_" + UniqueFileName + ".json" , Token);
...
inline std::deque<std::vector<std::uint8_t>> PersonalFileInfo::GenerateFileMultipleSubKeys( const PersonalFileInstance& Instance, const std::vector<uint8_t>& MasterKey )
{
	...
	if ( Instance.EncryptionAlgorithmNames.size() <= 4 && Instance.DecryptionAlgorithmNames.size() <= 4 )
	{
		...
		HTDP_ObjectArgument.NeedHashByteTokenSize = ( 256 / sizeof( uint8_t ) ) * Instance.DecryptionAlgorithmNames.size();
	...
	else
	{
		...
		{
			MultipleSubKeys.push_back( std::vector<uint8_t>( 256 / sizeof( uint8_t ), 0x00 ) );
		}
