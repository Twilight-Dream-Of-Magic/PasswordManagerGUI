...
inline std::string PasswordAndHash(const std::string& Password, std::string RandomSalt)
{
	...
	if(OptionalHashedHexadecimalString.has_value())
		...
		LogFatalHelper("If the size of the source string message is zero, then it cannot be transformed into the target hash digest message! ");
}
...
inline void MakePersonalFiles(const std::filesystem::path& UniquePathName_Password, const std::filesystem::path& UniquePathName_File, const std::string& Token)
{
	...
	if(!std::filesystem::is_directory(UniqueParentPathName_Password))
	{
		...
		{
			LogFatalHelper("Failed to create directory. Directory:{0}", std::make_tuple(UniqueParentPathName_Password.string()));
			return;
		}
	}
	...
	if(!std::filesystem::is_directory(UniqueParentPathName_File))
	{
		...
		{
			LogFatalHelper("Failed to create directory. Directory:{0}", std::make_tuple(UniqueParentPathName_File.string()));
			return;
		}
	}
	...
	{
		LogErrorHelper("File already exists. FilePath: {0}", std::make_tuple(UniquePathName_Password.string()));
		return;
	}
	else
	{
		...
		{
			file.close();
			LogInfoHelper("Personal password data file created: {0}", std::make_tuple(UniquePathName_Password.string()));
		}
		else
		{
			LogFatalHelper("Failed to create the personal password data file. FilePath: {0}", std::make_tuple(UniquePathName_Password.string()));
		}
	}

	if (std::filesystem::exists(UniquePathName_File))
	{
		LogErrorHelper("File already exists. FilePath: {0}", std::make_tuple(UniquePathName_File.string()));
		return;
	}
	else
	{
		...
		{
			file.close();
			LogInfoHelper("Personal data file created : {0}", std::make_tuple(UniquePathName_File.string()));
		}
		else
		{
			LogFatalHelper("Failed to create the personal data file. FilePath: {0}", std::make_tuple(UniquePathName_File.string()));
		}
	}
...
void PersonalPasswordInfo::Deserialization(const std::filesystem::path& FilePath)
{
	...
	if (!file.is_open())
	{
		LogFatalHelper("Failed to open the file for reading. FilePath: {0}", std::make_tuple( FilePath.string()) );
	}
	else
	{
...
)
{
	...
	{
		LogFatalHelper("HashMap_EncryptedSymmetricKey is empty!");
	}
...
void PersonalFileInfo::Deserialization(const std::filesystem::path& FilePath)
{
	...
	if (!file.is_open())
	{
		LogFatalHelper("Failed to open the file for reading: {0}", std::make_tuple(FilePath.string()));
		return;
	}
	else
	{
...
PersonalFileInfo::PersonalFileInstance& PersonalFileInfo::GetFileInstanceByID(uint64_t ID)
{
	...
	{
		LogErrorHelper("Error: PersonalFileInfo is empty.");
...
inline std::deque<std::vector<std::uint8_t>> PersonalFileInfo::GenerateFileMultipleSubKeys( const PersonalFileInstance& Instance, const std::vector<uint8_t>& MasterKey )
{
	...
	if ( Instance.EncryptionAlgorithmNames.size() <= 4 && Instance.DecryptionAlgorithmNames.size() <= 4 )
	{
		...
		{
			LogErrorHelper( "Failed to generate multiple subkeys." );
		}
...
bool PersonalFileInfo::EncryptFile( const std::string& Token, const PersonalFileInstance& Instance, const std::filesystem::path& SourceFilePath, const std::filesystem::path& EncryptedFilePath )
{
	...
	{
		LogErrorHelper( "Error: Encryption algorithm is not set." );
		LogErrorHelper( "Error: Decryption algorithm is not set." );
		return false;
	}
	...
	{
		LogWarnHelper( "Warning: Encryption algorithms and Decryption algorithms are not the same size." );
		return false;
	}
