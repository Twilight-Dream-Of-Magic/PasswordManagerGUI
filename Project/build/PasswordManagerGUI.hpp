...
void AsyncTask(std::atomic_bool& busy_flag, F task, Args&&... args)
{
	...
	{
		LOG_WRN("Task {} is already running, skipping...", typeid(task).name());
		return;
	}

	ScopeGuard<std::atomic_bool, void(std::atomic_bool&)> SG(busy_flag, [](std::atomic_bool& flag) { flag.store(false);});

	// todo: 更好的函数log信息	
	LOG_NTC("Task {} is running...", typeid(task).name());
	try
	{
	...
	{
		LOG_ERR("Error in {}: {}", typeid(task).name(),e.what());
	}
	catch (...)
	{
		LOG_ERR("Unknown exception in {}", typeid(task).name());
	}
};
	...
	if (ImGui::Button("Login"))
	{
		...
		if (LoadPasswordManagerUUID(CurrentUserKey))
		{
			if (CurrentUserKey.RandomSalt.empty() && CurrentUserKey.RandomPasswordSalt.empty() && CurrentUserKey.RegistrationTime == 0)
			{
				...
				LOG_WRN("Login failed because the UUID of the contents of the 'current_uuid.json' file is invalid!");
			...
			else
			{
				if (VaildUsername == false && VaildPassword == true)
				{
					...
					LOG_WRN("Failed to login, incorrect username by UUID checking");
				...
				else if (VaildUsername == true && VaildPassword == false)
				{
					...
					LOG_WRN("Failed to login, incorrect password by security comparison");
...
inline void Do_ChangeInstanceMasterKeyWithSystemPassword(std::vector<char>& BufferLoginPassword, ApplicationData& AppData)
{
	static const auto task_change_ins_mst_key_wth_sys_pwd = [](ApplicationData& AppData, const std::vector<char>& BufferLoginPassword)
	{
		...
		using AppD_t = ApplicationData;
		ScopeGuard <AppD_t> sg
		(
			AppData, [](AppD_t& AppData)
			{
				AppData.ShowPPI_Password = std::string(2048, 0x00);
				AppData.ShowPPI_NewPassword = std::string(2048, 0x00);
			}
