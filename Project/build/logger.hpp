...
inline void Logger::Print(Logger::Level level, const std::string_view message, uint32_t mask, const std::source_location& loc)
{
	...
	static constinit auto gen_src_info = [](bool show_color, const std::source_location& loc) -> std::string
	{
	...
	static const auto generate_text = [](uint32_t mask, Logger::Level level, const std::source_location& loc, const std::string_view message, bool show_color) -> std::stringstream
	{
		...
		if (mask & ~~Logger::Mask::SHOW_SRC_INFO)
			ss << gen_src_info((mask & ~~Logger::Mask::COLOR_SRC_INFO) && show_color, loc);
	...
	//	output_stream << generate_text(log_mask, level, loc, message, show_color).str();
	//if (_mask & ~~Logger::Mask::LOG_TO_FILE)
	//	LogToFile(generate_text(log_mask, level, loc, message, false).str());
...
inline bool Logger::RotateLogFile(uint32_t try_count, std::chrono::milliseconds t, uint32_t mask)
{
	for (uint32_t i = 0; i < try_count; ++i)
	{
		...
		//std::this_thread::sleep_for(t); todo:这里使用协程调度
	}
	return false;
}
