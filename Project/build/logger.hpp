...
private:
	...
	void DoLog(Level level,const std::string_view sv, const std::tuple<Types...>& tpl_args, const std::source_location& loc = std::source_location::current())
	{
		try
		{
			auto formatted = std::apply([&](const Types&... args)
			{
...
inline void Logger::Print( Logger::Level level, const std::string_view message, const std::source_location& loc)
{
	...
	using level_config_t = std::tuple<tag_str, std::string_view>;
	...
	static const auto get_tag = []( Logger::Level level ) -> tag_str
	{
		return std::get<0>(level_config[ enum_integer( level ) ]);
	};

	static const auto get_color = []( Logger::Level level ) -> color_str
	{
		return std::get<1>(level_config[ enum_integer( level ) ]);
	};

	static const auto gen_src_info = [](const std::source_location& loc) -> std::string 
	{
	...
	static const auto gen_text = [](Logger& self, Logger::Level level, const std::source_location& loc, const std::string_view message, bool show_color) -> std::stringstream
	{
		std::stringstream		ss;
		...
		if (self._mask & ~~Logger::Mask::SHOW_COLOR && show_color)
			ss << reset_color_str << get_color(level);
	...
	//todo: 把下面的部分封装为异步任务，值的方式捕获level和message，闭包，交给Log池
	...
	if (_mask & ~~Logger::Mask::LOG_TO_CONSOLE)
		output_stream << gen_text(*this, level, loc, message, true).str();
	if ( _mask & ~~Logger::Mask::LOG_TO_FILE )
		LogToFile(gen_text(*this, level, loc, message, false).str());

	// issue: 真的应该由logger来terminate吗？
