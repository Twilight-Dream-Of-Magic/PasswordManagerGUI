...
class Logger
{
  public:
	...
	struct LogTask
	{
		...
		template <typename SrcInfo, typename Msg>
		    requires std::constructible_from<Logger::SourceInfo, SrcInfo &&> && std::constructible_from<std::string, Msg &&>
		LogTask(SrcInfo &&src, Msg &&msg, std::underlying_type_t<Logger::Mask> msk, Logger::Level lvl) noexcept(
		    std::is_nothrow_constructible_v<Logger::SourceInfo, SrcInfo &&> && std::is_nothrow_constructible_v<std::string, Msg &&>)
	...
	void Log(
	    ...
	    const std::tuple<Types...>         &tpl_args,
	    uint32_t                            mask = 0u,
  ...
  private:
	...
	enum class GenLogTextProxy : uint32_t
	{
		LogToConsole,
		LogToFile,
		// LogToNetwork,
	...
	static constexpr uint32_t DEFAULT_MASK_VALUE = 0xFFFFFFFFu;
	...
	uint32_t                                     default_mask_ = DEFAULT_MASK_VALUE;
};
...
class Logger::DefaultFileSink : public Logger::LogSink
{
  ...
  private:
	...
	static constexpr size_t MAX_FILE_SIZE_VALUE = 1024 * 1024 * 64;
	std::ofstream         fs_;
...
inline uint32_t Logger::LogHelper::GetActiveMask() const { return ((local_mask__ ? local_mask__ : logger__.default_mask_) | mask_enabled__) & ~mask_disabled__; }
...
inline Logger::LogHelper::LogStream Logger::LogHelper::Stream() const { return LogHelper::LogStream(*this); }
...
inline Logger::LogHelper Logger::Helper(Logger::Level level, uint32_t mask, const std::source_location &loc) { return LogHelper(*this, level, mask, loc); }

inline Logger::LogHelper Logger::Normal(uint32_t mask, const std::source_location &loc) { return LogHelper(*this, Logger::Level::NORMAL, mask, loc); }

inline Logger::LogHelper Logger::Info(uint32_t mask, const std::source_location &loc) { return LogHelper(*this, Logger::Level::INFO, mask, loc); }

inline Logger::LogHelper Logger::Debug(uint32_t mask, const std::source_location &loc) { return LogHelper(*this, Logger::Level::DEBUG, mask, loc); }

inline Logger::LogHelper Logger::Notice(uint32_t mask, const std::source_location &loc) { return LogHelper(*this, Logger::Level::NOTICE, mask, loc); }

inline Logger::LogHelper Logger::Warning(uint32_t mask, const std::source_location &loc) { return LogHelper(*this, Logger::Level::WARNING, mask, loc); }

inline Logger::LogHelper Logger::Error(uint32_t mask, const std::source_location &loc) { return LogHelper(*this, Logger::Level::ERROR, mask, loc); }

inline Logger::LogHelper Logger::Fatal(uint32_t mask, const std::source_location &loc) { return LogHelper(*this, Logger::Level::FATAL, mask, loc); }
...
inline auto operator~(Logger::Mask a) -> std::underlying_type_t<Logger::Mask> { return (~std::underlying_type_t<Logger::Mask>(a)); }
...
inline void Logger::SetDefaultMaskFlag(Logger::Mask flag) { default_mask_ |= ~~flag; }

inline void Logger::UnsetDefaultMaskFlag(Logger::Mask flag) { default_mask_ &= ~flag; }
...
inline uint32_t Logger::GetDefaultMask() const { return default_mask_; }
...
inline void Logger::Print(Logger::Level level, const std::string_view message, uint32_t mask, const std::source_location &loc)
{
	...
	static auto async_log = [](Logger &self, Logger::Level level, const std::string_view message, uint32_t mask, const std::source_location &loc)
	{ self.AppendTask(Logger::LogTask(Logger::ParseSourceInfo(loc), message, mask, level)); };

	static auto sync_log = [](Logger &self, Logger::Level level, const std::string_view message, uint32_t mask, const std::source_location &loc)
	{
		std::array<Logger::LogTask, 1> task{Logger::LogTask(Logger::ParseSourceInfo(loc), message, mask, level)};
	...
	{
		sync_log(*this, level, message, mask, loc);
		// throw std::runtime_error(std::string("FATAL: ") + std::string(message));
	}
	else
	{
		async_log(*this, level, message, mask, loc);
	}
...
inline void Logger::DoLog(Level level, const std::string_view sv, const std::tuple<Types...> &tpl_args, uint32_t mask, const std::source_location &loc)
{
	try
	{
		const auto formatted = std::apply([&](const Types &...args) { return std::vformat(sv, std::make_format_args(args...)); }, tpl_args);
	...
	catch (const std::format_error &)
	{
		...
		ss << "format_error: ";
		ss << sv;
		std::apply([&](const Types &...args) { (ss << ... << args); }, tpl_args);
...
inline Logger::SourceInfo Logger::ParseSourceInfo(const std::source_location &loc)
{
	...
	SourceInfo       info{
	          .path            = (last_slash != std::string_view::npos) ? file_name.substr(0, last_slash) : ""sv,
	          .filename        = (last_slash != std::string_view::npos) ? file_name.substr(last_slash + 1) : file_name,
	          .function_prefix = {},
	          .function_core   = {},
	          .line            = loc.line()};
	...
	else if (std::regex_search(func_start, func_end, match, func_regex))
	{
		if (match[1].matched)
		{
			info.function_prefix = {match[1].first, static_cast<size_t>(match[1].length())};
		}
		if (match[2].matched)
		{
			info.function_core = {match[2].first, static_cast<size_t>(match[2].length())};
		}
	}
...
inline std::string Logger::GenerateLogText(const Logger::LogTask &task)
{
	...
	static constexpr auto enum_integer = [](Logger::Level level) -> size_t { return static_cast<std::underlying_type_t<Logger::Level>>(level); };
	...
	//[[maybe_unused]] static constexpr auto get_config = [](Logger::Level level) -> const decltype(level_config)::const_reference { return level_config.at(enum_integer(level)); };

	static const auto generate_text_with_color = [](uint32_t mask, Level level, const SourceInfo &src_info, std::string_view message) -> std::string
	{
		// 颜色常量
	...
	static const auto generate_text = [](uint32_t mask, Level level, const SourceInfo &src_info, std::string_view message) -> std::string
	{
		std::ostringstream ss{};
...
inline void Logger::BackgroundThreadLoop(std::stop_token st)
{
	...
	while (!st.stop_requested())
	{
		std::deque<std::vector<Logger::LogTask>> taskbuffers;
		{
			...
			cv_.wait_for(lock, 200ms, [this, &st]() -> bool { return st.stop_requested() || !backbuffers_.empty(); });
...
inline bool Logger::DefaultFileSink::RollToNewFile()
{
	...
	try
	{
		...
		const auto new_name   = filepath_.parent_path() / std::format("{}_{:%Y.%m.%d_%H.%M.%S}{}", filepath_.stem().string(), local_time, filepath_.extension().string());
