...
void GenerateUUID(std::vector<char> UserName, const std::string& RandomSalt, uint64_t& RegistrationTime, std::string& UUID)
{
...
void GenerateRandomSalt(std::string& RandomSalt)
{
	// Get the current timestamp
	auto Timestamp = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
...
void RegenerateMasterKey(PersonalPasswordInfo& PersonalPasswordInfo, const std::string& Token)
{
...
std::vector<unsigned char> HashUUID( const std::vector<unsigned char>& data, size_t size_limit )
{
...
std::string GenerateStringFileUUIDFromStringUUID(const std::string& UUID)
{
	//20 Bytes == 160 Bits
...
void SavePasswordManagerUser(const std::pair<PasswordManagerUserKey, PasswordManagerUserData>& NewUserData)
{
...
bool LoadPasswordManagerUUID(PasswordManagerUserKey& CurrentUserKey)
{
	nlohmann::json CurrentUUID;
...
void LoadPasswordManagerUser(const PasswordManagerUserKey& CurrentUserKey, PasswordManagerUserData& EmptyUserData)
{
...
std::string PasswordAndHash(const std::vector<char>& Password, std::string RandomSalt)
{
...
std::string PasswordAndHash(const std::string& Password, std::string RandomSalt)
{
	std::string PasswordString(Password.begin(), Password.end());
...
bool VerifyUUID(const std::vector<char>& Username, const std::string& RandomSalt, uint64_t& RegistrationTime, const PasswordManagerUserKey& CurrentUserKey)
{
	// Load existing all user data
	nlohmann::json CurrentUUID;
...
bool VerifyPassword(const std::vector<char>& Password, const PasswordManagerUserKey& CurrentUserKey, const PasswordManagerUserData& CurrentUserData)
{
...
std::string MakeTokenString(const std::string& UUID, const std::vector<char>& BufferLoginPassword)
{
...
std::string MakeTokenString(const std::string& UUID, const std::string& BufferLoginPassword)
{
	std::string PasswordString = std::string(BufferLoginPassword.begin(), BufferLoginPassword.end());
...
std::vector<std::uint8_t> GenerateMasterBytesKeyFromToken(const std::string& Token)
{
	using namespace CommonSecurity::SHA;
...
void MakePersonalFiles(const std::filesystem::path& UniquePathName_Password, const std::filesystem::path& UniquePathName_File, const std::string& Token)
{
...
void FirstLoginLogic(const std::vector<char>& BufferLoginPassword, const PasswordManagerUserKey& CurrentUserKey, PasswordManagerUserData& CurrentUserData)
{
...
std::vector<PersonalPasswordInfo::PersonalPasswordInstance>& PersonalPasswordInfo::GetPasswordInstances()
{
...
// 获取所有文件实例
auto PersonalFileInfo::GetFileInstanceByID(uint64_t ID) -> std::optional<std::reference_wrapper<PersonalFileInfo::PersonalFileInstance >>
{
...
std::deque<std::vector<std::uint8_t>> PersonalFileInfo::GenerateFileMultipleSubKeys( const PersonalFileInstance& Instance, const std::vector<uint8_t>& MasterKey )
{
	using namespace CommonSecurity::SHA;
...
bool PersonalFileInfo::DecryptFile( const std::string& Token, const PersonalFileInstance& Instance, const std::filesystem::path& EncryptedFilePath, const std::filesystem::path& DecryptedFilePath )
{
	...
	if (Instance.DecryptionAlgorithmNames.empty())
    {
		Logger::Instance().Fatal().Log("DecryptionAlgorithmNames is empty");
		std::terminate();
	}
