	...
	if (AppData.ShowPFI_CreateFileInstance)
	{
		if (ImGui::Begin("Create File Instance", &AppData.ShowPFI_CreateFileInstance, ImGuiWindowFlags_AlwaysAutoResize))
		{
			...
			if (ImGui::Button("Create File Instance"))
			{
				...
				// 创建文件实例
				auto FileInstance = AppData.PersonalFileObject.CreateFileInstance(
		...
		if (ImGui::Begin("Encrypt File", &AppData.ShowPFI_EncryptFile, ImGuiWindowFlags_AlwaysAutoResize))
		{
			...
			//if (!AppData.IsSourceFileSelected)
			{
				...
				FileDialogCallback(
				    ...
				    AppData.IsOriginalFileSelected,
					false,
				    ...
				    std::ref(AppData.IsOriginalFileSelected));
			}
			...
			//if (!AppData.IsEncryptedFileSelected)
			{
				...
				FileDialogCallback(
				    "Save Encrypted File",
				    AppData.IsTargetEncryptedFileSelected,
					true,
				    ...
				    std::ref(AppData.TargetEncryptedFilePath),
				    std::ref(AppData.IsTargetEncryptedFileSelected));
			}

			static bool Success = false;
			if (ImGui::Button("Encrypt"))
			{
				if (!AppData.SourceFilePath.empty() && !AppData.TargetEncryptedFilePath.empty())
				{
		...
		if (ImGui::Begin("Decrypt File", &AppData.ShowPFI_DecryptFile, ImGuiWindowFlags_AlwaysAutoResize))
		{
			...
			//if (!AppData.IsEncryptedFileSelected)
			{
				...
				FileDialogCallback(
					...
					AppData.IsSourceEncryptedFileSelected,
					false,
					[]( const std::filesystem::path& in_path, std::filesystem::path& out_path, bool& result )
					{
						out_path = in_path;
					...
					std::ref( AppData.SourceEncryptedFilePath ),
					std::ref( AppData.IsSourceEncryptedFileSelected ) 
				);
			}
			...
			{
				if (!AppData.SourceEncryptedFilePath.empty() && !AppData.DecryptedFilePath.empty())
				{
					std::string Password(BufferLoginPassword.begin(), BufferLoginPassword.end());
					...
					if (FileInstance.has_value())
					{
						Success = AppData.PersonalFileObject.DecryptFile(
						    MakeTokenString(AppData.UserKey.RandomUUID, Password),
						    FileInstance.value().get(),
						    AppData.SourceEncryptedFilePath,
