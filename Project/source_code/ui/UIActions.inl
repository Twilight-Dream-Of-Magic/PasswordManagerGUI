...
inline void RefillData_PersonalFiles()
{
	...
	{
		CurrentApplicationData.PersonalPasswordObject.Deserialization(CurrentApplicationData.PersonalPasswordInfoFilePath);
	}
	else
	{
		...
		CurrentApplicationData.PersonalPasswordObject.Serialization(CurrentApplicationData.PersonalPasswordInfoFilePath);
	}

	if (std::filesystem::exists(CurrentApplicationData.PersonalDataInfoFilePath))
	{
		CurrentApplicationData.PersonalFileObject.Deserialization(CurrentApplicationData.PersonalDataInfoFilePath);
	}
	else
	{
		...
		CurrentApplicationData.PersonalFileObject.Serialization(CurrentApplicationData.PersonalDataInfoFilePath);
	}
}
...
inline void Do_CreatePasswordInstance(std::vector<char> &BufferLoginPassword, ApplicationData &AppData)
{
	static const auto task_create_and_encrypt_password = [](ApplicationData &AppData, const std::vector<char> &BufferLoginPassword)
	{
		    ...
		    && !AppData.ShowPPI_DecryptionAlgorithms.empty() && ValidPassword)
		{
			...
			auto PasswordInstance = AppData.PersonalPasswordObject.CreatePasswordInstance(
			...
			AppData.PersonalPasswordObject.AppendPasswordInstance(PasswordInstance);
...
inline void Do_ChangePasswordInstance(std::vector<char> &BufferLoginPassword, ApplicationData &AppData)
{
	static const auto task_change_pwd_ins = [](ApplicationData &AppData, const std::vector<char> &BufferLoginPassword)
	{
		...
		if (!AppData.UserKey.RandomUUID.empty() && !BufferLoginPassword.empty() && !AppData.ShowPPI_ChangedPassword.empty() && !AppData.ShowPPI_EncryptionAlgorithms.empty()
		    && !AppData.ShowPPI_DecryptionAlgorithms.empty() && ValidPassword)
		{
			SetProgressTarget(AppData, 0.2f, 0.3f);

			auto new_end = std::find_if(AppData.ShowPPI_ChangedPassword.rbegin(), AppData.ShowPPI_ChangedPassword.rend(), [](char character) { return character != '\x00'; });

			AppData.ShowPPI_ChangedPassword.erase(new_end.base(), AppData.ShowPPI_ChangedPassword.end());
			...
			bool IsChanged = AppData.PersonalPasswordObject.ChangePasswordInstance(
			    AppData.ShowPPI_SelectedPasswordInstanceID,
			    AppData.ShowPPI_Description,
			    AppData.ShowPPI_ChangedPassword,
			    AppData.ShowPPI_EncryptionAlgorithms,
			    AppData.ShowPPI_DecryptionAlgorithms,
			    MakeTokenString(AppData.UserKey.RandomUUID, BufferLoginPassword),
			...
			// Clear Application GUI State Data
			memory_set_no_optimize_function<0x00>(AppData.ShowPPI_ChangedPassword.data(), AppData.ShowPPI_ChangedPassword.size());
...
inline void Do_DecryptionAllPasswordInstance(std::vector<char> &BufferLoginPassword, ApplicationData &AppData)
{
	...
	{
		AppData.PersonalPasswordObject.Deserialization(AppData.PersonalPasswordInfoFilePath);

		SetProgressTarget(AppData, 0.1f, 0.95f);

		// 调用ListAllPasswordInstance函数来执行列出密码实例的操作
		AppData.PersonalPasswordObject.ListAllPasswordInstance(MakeTokenString(AppData.UserKey.RandomUUID, BufferLoginPassword));
...
inline void Do_FindPasswordInstanceByID(std::vector<char> &BufferLoginPassword, ApplicationData &AppData)
{
	static const auto task_find_pwd_ins_by_id = [](ApplicationData &AppData, const std::vector<char> &BufferLoginPassword)
	{
		...
		auto Optional
		    = AppData.PersonalPasswordObject.FindPasswordInstanceByID(MakeTokenString(AppData.UserKey.RandomUUID, BufferLoginPassword), AppData.ShowPPI_SelectedPasswordInstanceID);
...
inline void Do_FindPasswordInstanceByDescription(std::vector<char> &BufferLoginPassword, ApplicationData &AppData)
{
	static const auto task_find_pwd_ins_by_desc = [](ApplicationData &AppData, const std::vector<char> &BufferLoginPassword)
	{
		...
		auto Optional = AppData.PersonalPasswordObject.FindPasswordInstanceByDescription(
		    MakeTokenString(AppData.UserKey.RandomUUID, BufferLoginPassword),
...
inline void Do_ChangeInstanceMasterKeyWithSystemPassword(std::vector<char> &BufferLoginPassword, ApplicationData &AppData)
{
	...
	{
		auto new_end = std::find_if(AppData.ShowPPI_ConfirmPassword.rbegin(), AppData.ShowPPI_ConfirmPassword.rend(), [](char character) { return character != '\x00'; });

		AppData.ShowPPI_ConfirmPassword.erase(new_end.base(), AppData.ShowPPI_ConfirmPassword.end());
		...
		volatile auto SG = MakeScopeGuard(
		    [](ApplicationData &AppData, char ch, size_t size = 2048) // 2028是默认值所以可以不传
		    {
			    AppData.ShowPPI_ConfirmPassword    = std::string(size, ch);
			    AppData.ShowPPI_NewPassword = std::string(size, ch);
		    },
		    std::ref(AppData), // 注意，这里传ref，因为上面要&
		    static_cast<char>(0x00));

		if (!AppData.ShowPPI_ConfirmPassword.empty() && !AppData.ShowPPI_NewPassword.empty())
		{
			// Verify Password
			const bool ValidPassword = VerifyPassword(BufferLoginPassword, AppData.UserKey, AppData.UserData)
			                           && std::equal(AppData.ShowPPI_ConfirmPassword.begin(), AppData.ShowPPI_ConfirmPassword.end(), Password.begin(), Password.end());
			...
			if (ValidPassword)
			{
				...
				AppData.PersonalPasswordObject.ChangeInstanceMasterKeyWithSystemPassword(
				    AppData.PersonalPasswordInfoFilePath,
				    AppData.UserKey.RandomUUID + AppData.ShowPPI_ConfirmPassword,
